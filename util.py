...
#util.py is a class that contains some data structures for the actual game implementing (stack, queue)
...
#priority queue data structure, each inserted item has a priority with it
class PriorityQueue:
    ...
    #if the item is already in priority queue with higher priority, update its priority and rebuild the heap
    #if the item is already in priority queue with equal or lower priority, do nothing
    #if item is not in priority queue, do the same thing as self.push
...
#priority queue data structure with the same push/pop of the queue and stack
...
#counter keeps track of counts for a set of keys
class Counter(dict):
    ...
    #returns a list of keys sorted by their values, key with the heighest values will appear first
    def sortedKeys(self):
    ...
    #returns the sum of counts for all the keys
    def totalCount(self):
    ...
    #adds a counter to another counter, increments the current counter by the values stored in the second counter
    def __radd__(self, y):
...
#turns a matrix into a list of coordinates
